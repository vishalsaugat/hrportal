                ...
                <table class="mdl-data-table mdl-js-data-table mdl-data-table--selectable mdl-shadow--2dp">
                    ...
                    <tbody>
                        ...
                        <tr>
                                        ...
                                        <td><?= $user->has('reporting_user') ? $this->Html->link($user->reporting_user->first_name." ".$user->reporting_user->last_name, ['controller' => 'Users', 'action' => 'view', $user->reporting_user->id]) : '' ?></td>
                                        <td><?= h($user->created_at) ?></td>
                                        <td><?= h($user->updated_at) ?></td>
                                        <td><?= $this->Number->format($user->status) ?></td>
                                        <td class="actions">
                                <?= $this->Html->link("<i class='material-icons'>link</i>", ['action' => 'view', $user->id],['escape'=>false]) ?>
                                <?= $this->Html->link( "<i class='material-icons'>edit</i>", ['action' => 'edit', $user->id],['escape'=>false]) ?>
                                <?= $this->Form->postLink( "<i class='material-icons'>delete</i>", ['action' => 'delete', $user->id], ['confirm' => __('Are you sure you want to delete # {0}?', $user->id),'escape'=>false]) ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
